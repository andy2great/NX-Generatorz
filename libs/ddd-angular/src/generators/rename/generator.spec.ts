...
const DDDObjects = {
  ...
  API: {
    dddClass: API,
    name: `${defaultOptions.domain}-api-${defaultOptions.name}`,
  },
  Feature: {
    dddClass: Feature,
    name: `${defaultOptions.domain}-feature-${defaultOptions.name}`,
  },
  Shell: {
    dddClass: Shell,
    name: `${defaultOptions.domain}-shell-${defaultOptions.name}`,
  },
...
describe('rename generator', () => {
  ...
  describe.each(Object.values(DDDObjects))(
    ...
    (definition) => {
      it('should update the project name in the angular.json', async () => {
        ...
        const DDDObject = await new definition.dddClass(
          appTree,
          definition.name
        );
        ...
        expect(angularJson.projects).toHaveProperty(DDDObject.project);
      });
